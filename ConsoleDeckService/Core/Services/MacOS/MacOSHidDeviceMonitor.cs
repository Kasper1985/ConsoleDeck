...
/// <summary>
/// macOS implementation of HID device monitoring using HidSharp.
/// Detects ConsoleDeck devices and monitors for key presses.
/// </summary>
public class MacOSHidDeviceMonitor(ILogger<MacOSHidDeviceMonitor> logger, IConfigurationService configService) : IHidDeviceMonitor
{
    ...
    private async Task ConnectToDeviceAsync(HidDevice device)
    {
        try
        {
            ...
            _currentStream.ReadTimeout = 1000;
    ...
    private void ProcessHidReport(byte[] buffer, int length)
    {
        ...
        // Byte 0: Report ID (REPORT_ID_CONSUMER_CONTROL = 2)
        // Byte 1: Keycode of the pressed key
        // Byte 2: Reserved (FF - key press, 00 - key release)
        var code = buffer[0];
